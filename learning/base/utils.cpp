...
namespace roobtyan {
    ...
    absl::Status string_to_time(const std::string &str, absl::Time *time) {
        std::string err;
        bool success = absl::ParseTime(absl::RFC3339_sec, str, time, &err);
    ...
    absl::Time get_current_time_str_only_day() {
        ...
        absl::LoadTimeZone("Asia/Shanghai", &local);
        auto now = absl::Now();
        auto today = absl::ToCivilDay(now, local);
    ...
    absl::Time get_time_only_day(absl::Time time) {
        absl::TimeZone local;
        absl::LoadTimeZone("Asia/Shanghai", &local);
        auto today = absl::ToCivilDay(time, local);
        return absl::FromCivil(today, local);
    }
