...
void MemoryTool::AddMemory() {
  ...
  memory_items_.emplace_back(question, answer);
}
...
void MemoryTool::ReviewMemory() {
  ...
  for (const auto& item : memory_items_) {
  ...
  for (auto& item : today_review_items) {
    std::cout << "Question(" << index++ << "/" << today_review_items.size() << ")" << " " << item.
        question
        << "\n";
    std::string answer;
    std::cout << "Enter answer: ";
    ...
    if (answer == item.answer) {
      std::cout << "Correct!\n";
      item.correct_count++;
      item.next_review_time = get_time_str_with_offset(
          GetMemoryItemNextReviewGap(item.correct_count));
    } else {
      std::cout << "Incorrect!\n";
  ...
  for (const auto& item : today_review_items) {
    for (auto& memory_item : memory_items_) {
...
void MemoryTool::LoadData() {
  std::ifstream inFile(memory_data_path_);
  if (!inFile) return;

  auto memory_item = MemoryItem();
  while (inFile >> memory_item.uuid >> memory_item.question >> memory_item.answer >> memory_item.
         review_count >>
    ...
    memory_items_.push_back(memory_item);
  }

  inFile.close();
}

void MemoryTool::SaveData() const {
  std::ofstream outFile(memory_data_path_);
  for (const auto& item : memory_items_) {
...
MemoryTool::MemoryItem::MemoryItem(std::string question, std::string answer)
  ...
  // 默认第一次复习时间为24小时后
  const auto in_next_review_time = get_current_time() + absl::Hours(24);
  std::string next_review_time_str;
  if (const auto status = time_to_string(in_next_review_time, &next_review_time_str); !status.
    ok()) {
